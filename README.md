...
└── front-end/
    ...
    └── src/
        ...
        │   └── index.tsx         # "/" -> UserList
        └── components/           # one folder per component: .tsx + .styles.ts
            ├── AppShell/         # header + <Outlet /> page chrome
            ├── UserList/         # fetch button, loading/error/empty states
            └── UserCard/         # single user's name, email, join date
```
